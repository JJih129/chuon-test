...
namespace LWGUI
{
	...
	public class SubToggleOffDrawer : SubDrawer
	{
		private string _keyWord = String.Empty;
		...
		public override void Apply(MaterialProperty prop)
		{
			base.Apply(prop);
			if (!prop.hasMixedValue && IsMatchPropType(prop))
				Helper.SetShaderKeyWord(prop.targets, Helper.GetKeyWord(_keyWord, prop.name), prop.floatValue <= 0.0f);
		}
	}
	...
	public class TitleKeywordEnumDrawer : SubDrawer
	{
		private GUIContent[] _names;
		private string[]     _keyWords;
		private float[]      _values;
		...
		public TitleKeywordEnumDrawer(string n1, string k1)
			: this("_", new string[1] { n1 }, new string[1] { k1 }) { }
		
		public TitleKeywordEnumDrawer(string n1, string k1, string n2, string k2)
			: this("_", new string[2] { n1, n2 }, new string[2] { k1, k2 }) { }

		public TitleKeywordEnumDrawer(string n1, string k1, string n2, string k2, string n3, string k3)
			: this("_", new string[3] { n1, n2, n3 }, new string[3] { k1, k2, k3 }) { }

		public TitleKeywordEnumDrawer(string n1, string k1, string n2, string k2, string n3, string k3, string n4, string k4)
			: this("_", new string[4] { n1, n2, n3, n4 }, new string[4] { k1, k2, k3, k4 }) { }

		public TitleKeywordEnumDrawer(string n1, string k1, string n2, string k2, string n3, string k3, string n4, string k4, string n5, string k5)
			: this("_", new string[5] { n1, n2, n3, n4, n5 }, new string[5] { k1, k2, k3, k4, k5 }) { }

		public TitleKeywordEnumDrawer(string group, string n1, string k1)
			: this(group, new string[1] { n1 }, new string[1] { k1 }) { }

		public TitleKeywordEnumDrawer(string group, string n1, string k1, string n2, string k2)
			: this(group, new string[2] { n1, n2 }, new string[2] { k1, k2 }) { }

		public TitleKeywordEnumDrawer(string group, string n1, string k1, string n2, string k2, string n3, string k3)
			: this(group, new string[3] { n1, n2, n3 }, new string[3] { k1, k2, k3 }) { }

        public TitleKeywordEnumDrawer(string group, string n1, string k1, string n2, string k2, string n3, string k3, string n4, string k4)
            : this(group, new string[4] { n1, n2, n3, n4 }, new string[4] { k1, k2, k3, k4 }) { }

        public TitleKeywordEnumDrawer(string group, string n1, string k1, string n2, string k2, string n3, string k3, string n4, string k4, string n5, string k5)
            : this(group, new string[5] { n1, n2, n3, n4, n5 }, new string[5] { k1, k2, k3, k4, k5 }) { }
		#endregion

		public TitleKeywordEnumDrawer(string group, string[] names, string[] keyWords = null, float[] values = null)
		{
			Init(group, names, keyWords, values);
		}
		...
		public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
		{
			...
			var myStyle = new GUIStyle(EditorStyles.popup);
			myStyle.fontSize = 15;

			//myStyle.padding = new RectOffset(10, 0, 0, 0);
			myStyle.alignment = TextAnchor.LowerLeft;
			Helper.AdaptiveFieldWidth(myStyle, _names[index]);
			int newIndex = EditorGUI.Popup(rect, label, index, _names, myStyle);
			EditorGUI.showMixedValue = false;
			if (Helper.EndChangeCheck(lwgui, prop))
			{
				prop.floatValue = _values[newIndex];
	...
	public class UVChannelDrawer : SubDrawer
	{
		private static GUIContent[] _names = new[] {
			new GUIContent("UV0"),
			new GUIContent("UV1"),
		...
		public UVChannelDrawer() { }
		public UVChannelDrawer(string group)
		{
			this.group = group;
		}
	...
	{
		protected static readonly string DefaultRootPath = "Assets.ASP";
		...
		protected virtual void OnEditRampMap(Texture texture, GradientObject gradientObject)
		{
			if (texture != null && gradientObject != null)
			{
				if (gradientObject.gradient.mode == GradientMode.Fixed && texture.filterMode != FilterMode.Point)
				{
					var unityPath = AssetDatabase.GetAssetPath(texture);
					var textureImporter = AssetImporter.GetAtPath(unityPath) as TextureImporter;
					textureImporter.filterMode = FilterMode.Point;
					textureImporter.SaveAndReimport();
				}
				else if (gradientObject.gradient.mode != GradientMode.Fixed && texture.filterMode == FilterMode.Point)
				{
					var unityPath = AssetDatabase.GetAssetPath(texture);
					var textureImporter = AssetImporter.GetAtPath(unityPath) as TextureImporter;
		...
		public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
		{
			...
			if (hasGradientChanges || doSaveGradient)
			{
				...
				OnEditRampMap(prop.textureValue, gradientObject);
			}
	...
	public class ActiveIfDecorator : SubDrawer
	{
		...
		public class NameIfDecorator : SubDrawer
	{
		private NameIfData _nameIfData = new NameIfData();
		private readonly Dictionary<string, string> _compareFunctionLUT = new Dictionary<string, string>()
		{
			{ "Less",			"Less" },
			{ "L",				"Less" },
		...
		public NameIfDecorator(string propName, string comparisonMethod, float value, string overrideName) : this("And", propName, comparisonMethod, value, overrideName) { }
		public NameIfDecorator(string logicalOperator, string propName, string compareFunction, float value, string overrideName)
		{
			_nameIfData.logicalOperator = logicalOperator.ToLower() == "or" ? LogicalOperator.Or : LogicalOperator.And;
			_nameIfData.targetPropertyName = propName;
			_nameIfData.overrideDisplayName = overrideName.Replace("_", " / ");
			if (!_compareFunctionLUT.ContainsKey(compareFunction) || !Enum.IsDefined(typeof(CompareFunction), _compareFunctionLUT[compareFunction]))
				Debug.LogError("Invalid compareFunction: '" + compareFunction + "', Must be one of the following: Less (L) | Equal (E) | LessEqual (LEqual / LE) | Greater (G) | NotEqual (NEqual / NE) | GreaterEqual (GEqual / GE).");
			else
				_nameIfData.compareFunction = (CompareFunction)Enum.Parse(typeof(CompareFunction), _compareFunctionLUT[compareFunction]);
			_nameIfData.value = value;
		}
		...
		public override void BuildStaticMetaData(Shader inShader, MaterialProperty inProp, MaterialProperty[] inProps, PropertyStaticData inoutPropertyStaticData)
		{
			inoutPropertyStaticData.nameIfDatas.Add(_nameIfData);
		}
