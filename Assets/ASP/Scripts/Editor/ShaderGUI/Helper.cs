...
namespace LWGUI
{
	...
	public class Helper
	{
		...
		public static bool BeginPropertyFlags;
		...
		{
				var sealedClassType = typeof(MaterialProperty);
				var fieldInfo = sealedClassType.GetField("s_PropertyStack", BindingFlags.NonPublic | BindingFlags.Static);
				if (fieldInfo != null)
				{
					IList propertyStack = fieldInfo.GetValue(null) as IList;
					if (propertyStack != null && propertyStack.Count > 0)
					{
#if UNITY_2022_1_OR_NEWER
		...
		private static GUIContent _guiLanguageSelect = new GUIContent("", _iconVisibility, "Language Select");
		...
		public static void DrawToolbarButtons(ref Rect toolBarRect, LWGUI lwgui)
		{
			...
			buttonRect.x += buttonRectOffset;
			toolBarRect.xMin += buttonRectOffset;
			if (GUI.Button(buttonRect, _guiLanguageSelect, Helper.guiStyles_IconButton))
			{
				string[] displayModeMenus = new[]
				{
					"Show All Advanced	(" + lwgui.perShaderData.displayModeData.advancedCount + " of " + lwgui.perShaderData.propertyDatas.Count + ")",
